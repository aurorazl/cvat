...
(() => {
        ...
        _validateStateBeforeSave(frame, data, updated) {
            ...
            if (updated.color) {
                ...
                if (!/^#[0-9A-F]{6}$/i.test(data.color)) {
                    throw new ArgumentError(i18next.t('Got invalid color value: "${data.color}"').replace('${data.color}', `${data.color}`));
                }
            }
    ...
    class Shape extends Drawn {
        ...
        get(frame) {
            if (frame !== this.frame) {
                throw new ScriptingError(i18next.t('Got frame is not equal to the frame of the shape'));
            }
    ...
    class Tag extends Annotation {
        ...
        save(frame, data) {
            if (frame !== this.frame) {
                throw new ScriptingError(i18next.t('Got frame is not equal to the frame of the tag'));
            }
