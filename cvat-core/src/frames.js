...
(() => {
    ...
    function getFrameMeta(taskID, frame) {
        ...
        if (mode === 'interpolation') {
            ...
            if (frame >= meta.size) {
                throw new ArgumentError(i18next.t("Meta information about frame ${frame} can't be received from the server", {frame: `${frame}`}));
            } else {
                size = meta.frames[frame];
            }
        } else {
            throw new DataError(i18next.t('Invalid mode is specified ${mode}', {mode: `${mode}`}));
        }
        return size;
    }
